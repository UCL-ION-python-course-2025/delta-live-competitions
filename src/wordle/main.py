...
from wordle.tournament import WorldleTournament
...
def main() -> None:
    ...
    for entry in os.scandir(competitor_code_dir):
        ...
        if entry.is_file() and entry.name.startswith("team_") and entry.name.endswith(".py"):
            ...
            try:
                func = getattr(mod, "guess_word")
            except AttributeError as e:
                raise Exception(f"No function 'guess_word' found in file {file_name}") from e
            try:
    ...
    controller = CompetitionController(
        "wordle", teams, WordleGame, min_time_per_step=1, moves_per_state_change=6
    )
    tournament_view = WorldleTournament(controller)
