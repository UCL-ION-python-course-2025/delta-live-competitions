...
import delta_othello
from competition_controller import CompetitionController
from delta_othello.game_mechanics import load_network
from play_competition import play_competition
from team import Team

# makes e.g. "import game_mechanics" available to competitor code files
sys.path.append(delta_othello.__path__[0])  # isort:skip
from othello.game import OthelloGame  # isort:skip
from othello.visuals import OthelloGameViewer  # isort:skip
...
def main() -> None:
    ...
    for entry in os.scandir(competitor_code_dir):
        ...
        if entry.is_file() and entry.name != "__init__.py" and entry.name.endswith(".py"):
            ...
            if not file_name.startswith("team_"):
                continue
            mod = __import__(f"othello.competitor_code.{file_name}", fromlist=["None"])
            try:
    ...
    controller = CompetitionController(
        "Delta Academy Othello Competition",
        [
    ...
    view = OthelloGameViewer(controller=controller)
